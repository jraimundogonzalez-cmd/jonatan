# 26 · TradePilot Quant Engine — El Núcleo Matemático
...
> **Nomenclatura oficial (aprobada por el fundador)**: este componente se llamó "R Engine" hasta la aprobación de este capítulo. Pasa a llamarse **TradePilot Quant Engine** ("Quant Engine" en el resto del texto, siguiendo el mismo patrón sin prefijo que Rule Engine/Risk Engine) porque su alcance a largo plazo excede el cálculo sobre R — es, y se documenta como tal desde ahora, el motor cuantitativo de todo el producto. El nombre se actualizó en todo el blueprint donde aparecía (incluyendo el identificador de paquete `r-engine` → `quant-engine`, 05 §2/§5); ningún contrato, fórmula o decisión cambia con el renombrado, solo el nombre.

## 0. Aclaración que condiciona todo el capítulo: Quant Engine ≠ Risk Engine

22.5 §2.3 definió Risk Engine como el módulo que "computa, de forma determinista, magnitudes de riesgo". Este capítulo lo precisa un nivel más abajo, y la precisión importa: **Quant Engine es la capa de fórmulas puras; Risk Engine es quien la invoca, persiste sus resultados y emite eventos** (`CapitalRecalculado`, 21.5 §7). Quant Engine no sabe qué es una Cuenta, no sabe guardar nada, no emite eventos — recibe números y configuración, devuelve números. Risk Engine es, a partir de este capítulo, un **consumidor** de Quant Engine, no un sinónimo. Esta distinción ya estaba insinuada en 25 §2 (`quant-engine` como librería compartida, distinta del "núcleo síncrono" de Risk Engine dentro de Core Service) — aquí se formaliza como el contrato completo que el fundador pide, y se corrige 22.5 §2.3 para reflejarlo explícitamente (§8).

## 1. Contrato del Quant Engine
...
**¿Cómo se garantiza la reproducibilidad histórica?** El patrón Snapshot (19 regla 13) congela los *inputs*; `algorithm_version` (15 §3.5) congela qué *fórmula* se usó, solo donde la fórmula tiene una versión de diseño (arriba). Con ambos fijados, cualquier cálculo de Quant Engine es reproducible en cualquier momento futuro — coherente con I2 (23), corregido en su momento para exigir exactamente este matiz.

**¿Qué operaciones son puramente matemáticas?** Todo el catálogo de §2, por definición.

**¿Qué operaciones requieren contexto externo?** Ninguna, a propósito. Si algo necesita el nombre de una empresa, un límite configurado, o la hora actual del sistema, no pertenece a Quant Engine — pertenece a quien lo llama. Es Dependency Inversion aplicado por tercera vez en el blueprint (tras Rule Engine/Risk Engine en 22.5 y Snapshot Engine en 22.5 §2.11): Quant Engine no depende de nada, todo el sistema depende de Quant Engine.
...
**Aclaración de frontera, pedida explícitamente por el fundador ("Optimizer" como consumidor de Quant Engine, no como parte de él)**: Quant Engine provee la fórmula de evaluación de **una** configuración candidata. La búsqueda entre miles de configuraciones (el bucle de 02 §5.1) es responsabilidad del componente Optimizer, que llama a Quant Engine una vez por candidato — Quant Engine nunca sabe que existe una búsqueda en curso.
...
**¿Qué haría Apple para simplificar este capítulo?** Insistiría en que el catálogo completo (§2) nunca debe ser visible como tal para el usuario — son ~20 funciones para el equipo, y deberían traducirse en 5-6 números que el trader ve realmente en una pantalla (03 §4, 18 §6) sin saber que existe un "Quant Engine" detrás.

**¿Qué haría Linear para hacerlo más rápido?** Confirmaría la separación Grupo F (Optimizer llama a Quant Engine repetidamente) como la decisión correcta para no bloquear la experiencia — cada llamada a `simularGestion`/`calcularScore` es barata y determinista, así que miles de llamadas en un grid search (02 §5.1) siguen siendo instantáneas.
