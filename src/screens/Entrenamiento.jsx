...
function PRModal({ exercise, history, onSave, onClose }) {
  ...
  const initSets = () => {
    ...
    return [
      { weight: Math.round(w * 0.65), reps: baseR + 4 },
        ...
        {!advice && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "28px 1fr 1fr 28px", gap: 6, marginBottom: 6, padding: "0 2px" }}>
              ...
              <div style={{ fontSize: 9, color: "#64748b", fontWeight: 700, textAlign: "center", textTransform: "uppercase" }}>Reps hechas</div>
              <div />
            </div>
            ...
            {setsData.map((s, i) => {
              ...
              return (
                <div key={i} style={{ marginBottom: 10 }}>
                  ...
                  {prevSet && (
                    <div style={{ fontSize: 10, color: "#334155", paddingLeft: 34, marginTop: 3 }}>
                      Anterior: {prevSet.weight ? `${prevSet.weight}kg` : "—"} × {prevSet.reps || "—"}
                      {s.weight && prevSet.weight && +s.weight > +prevSet.weight && (
                        <span style={{ color: "#4ade80", marginLeft: 4 }}>↑ +{(+s.weight - +prevSet.weight).toFixed(1)}kg</span>
                      )}
                    </div>
                  )}
                </div>
                      ...
                      <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                        {h.setsData.map((hs, j) => (
                          <span key={j} style={{ background: "rgba(245,158,11,0.08)", borderRadius: 8, padding: "2px 8px", color: "#f59e0b", fontFamily: "'DM Mono',monospace" }}>
                            {hs.weight ? `${hs.weight}kg` : "—"}×{hs.reps || "—"}
                          </span>
                        ))}
                      </div>
                    ) : (
            ...
            <button onClick={handleSave} disabled={!hasAnyData}
              style={{ ...C.btnA, opacity: hasAnyData ? 1 : 0.4 }}>
              Guardar y analizar {numSets} series
            </button>
          </>
        )}
      </div>
...
const EX_PHOTO = {
  ...
  press_pecho_maquina:        ["chest press machine gym man seated", "machine chest press exercise"],
  aperturas_mancuernas:       ["dumbbell chest fly bench gym", "pec fly dumbbell"],
  aperturas_polea:            ["cable chest fly crossover gym man", "cable crossover fly chest"],
  ...
  pullover_polea:             ["straight arm cable pulldown man gym", "cable straight arm pulldown back man"],
  face_pull:                  ["face pull cable rear delt gym man", "cable face pull exercise"],
  remo_maquina:               ["seated row machine gym man back", "cable row machine seated gym"],
  // Hombros
  ...
  press_arnold:               ["arnold press dumbbell seated gym", "arnold dumbbell shoulder press rotation"],
  press_hombro_maquina:       ["shoulder press machine gym man seated", "machine overhead press gym"],
  elevaciones_laterales:      ["lateral raise dumbbell gym man", "dumbbell lateral raise shoulder"],
  elevaciones_polea:          ["cable lateral raise shoulder gym man", "cable shoulder lateral raise"],
  ...
  encogimientos_barra:        ["barbell shrug trap exercise gym", "barbell shrug shoulders trapezius"],
  remo_alto_barra:            ["upright barbell row shoulders trap gym", "upright row barbell exercise"],
  // Bíceps
  ...
  curl_predicador:            ["preacher curl EZ bar gym man", "scott curl bicep machine"],
  curl_concentrado:           ["concentration curl dumbbell seated man", "seated dumbbell curl bicep"],
  ...
  curl_femoral_sentado:       ["seated leg curl machine gym hamstring", "seated hamstring curl machine"],
  ...
  peso_muerto_mancuernas:     ["dumbbell romanian deadlift gym man", "dumbbell rdl hamstring legs"],
  hip_thrust:                 ["barbell hip thrust glute bridge gym man", "hip thrust barbell glute exercise"],
  glute_bridge:               ["glute bridge floor bodyweight exercise man", "hip thrust floor bodyweight glute"],
  patada_gluteo:              ["cable glute kickback machine gym", "donkey kick glute cable machine"],
  elevacion_gemelo_pie:       ["standing calf raise machine gym man", "calf raise exercise"],
  elevacion_gemelo_sentado:   ["seated calf raise machine gym man", "seated calf raise"],
  ...
  // Pecho — aislamiento que NO debe mostrar press de banca
  aperturas_inclinadas_mancuernas: ["incline dumbbell fly bench gym man", "incline chest fly dumbbell"],
  cruce_poleas_alto_bajo:          ["cable crossover standing chest gym man", "high to low cable fly chest"],
  cruce_poleas_bajo_alto:          ["low to high cable crossover chest gym", "cable fly standing chest gym"],
  pullover_mancuerna_pecho:        ["dumbbell pullover bench lying gym man", "lying dumbbell pullover chest"],
  flexiones_manos_elevadas:        ["incline push up hands on bench man", "push up elevated hands gym"],
  flexiones_pies_elevados:         ["decline push up feet elevated bench man", "push up feet on bench gym"],
  flexiones_triceps_manos_estrechas: ["close grip push up narrow hands floor man", "diamond hand push up tricep"],
  // Movimientos con poca cobertura de fallback genérico
  dragon_flag:                ["dragon flag exercise core gym man", "advanced core hold gymnastics"],
  sissy_squat:                ["sissy squat bodyweight quad exercise", "sissy squat leaning back gym"],
  wall_sit:                   ["wall sit exercise quad isometric", "wall squat hold legs"],
  glute_ham_raise:             ["glute ham raise machine gym", "GHR machine hamstring exercise"],
  bird_dog:                    ["bird dog exercise core floor man", "quadruped bird dog stability"],
  dead_hang_barra:             ["dead hang pull up bar man gym", "hanging from bar grip exercise"],
  plate_pinch_hold:             ["plate pinch grip hold gym man", "weight plate pinch grip training"],
  wrist_roller:                 ["wrist roller forearm exercise gym", "forearm roller device gym"],
  farmer_walk_trap_bar:         ["farmer walk trap bar gym man", "trap bar carry exercise gym"],
  hollow_rock:                  ["hollow body hold rock core floor", "hollow body position gymnastics core"],
  bear_plank:                   ["bear plank core exercise floor", "bear hold knees off floor core"],
  copenhague_plancha:            ["copenhagen plank adductor exercise bench", "side plank leg on bench adductor"],
  sentadilla_pistol:             ["pistol squat single leg bodyweight", "one leg squat exercise gym"],
  sentadilla_zercher_barra:      ["zercher squat barbell front gym man", "zercher squat elbows barbell"],
  v_squat_maquina:               ["V squat machine gym legs", "v-squat machine exercise gym"],
  lenador_polea_alto_bajo:        ["cable woodchopper high to low gym man", "woodchopper cable core rotation"],
  lenador_polea_bajo_alto:        ["cable woodchopper low to high gym man", "woodchopper cable core rotation up"],
  pallof_press_split_stance:      ["pallof press split stance cable core", "anti rotation cable press split stance"],
  crunch_polea:                   ["cable crunch kneeling gym abs", "kneeling cable crunch core"],
  crunch_maquina:                 ["abdominal crunch machine gym seated", "ab crunch machine exercise"],
  crunch_bicicleta:               ["bicycle crunch exercise floor abs", "bicycle crunch core workout"],
  crunch_lateral_suelo:           ["side crunch floor exercise oblique", "oblique crunch floor core"],
  buenos_dias_barra:              ["good morning barbell exercise gym man", "barbell good morning hamstring back"],
  good_morning_mancuernas:        ["dumbbell good morning exercise gym", "good morning dumbbell hip hinge"],
};
