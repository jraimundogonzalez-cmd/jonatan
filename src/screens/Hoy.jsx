...
function FoodScanner({ todayLog, setTodayLog }) {
  ...
  return (
    <div style={{ ...S.card, borderColor: "rgba(239,68,68,0.2)", background: "rgba(239,68,68,0.025)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }} onClick={() => setOpen(o => !o)}>
        <div>
          <span style={S.lbl}>📷 Analizar comida con IA</span>
          {scanned.length > 0 && (
            <div style={{ fontSize: 12, color: "#94a3b8", marginTop: -4 }}>
              {scanned.length} alimento{scanned.length > 1 ? "s" : ""} · {totals.kcal} kcal registradas
            </div>
          )}
        </div>
          ...
          {!apiKey ? (
            <div>
              <div style={{ fontSize: 12, color: "#94a3b8", marginBottom: 12, lineHeight: 1.65 }}>
                Necesitas una API key de Anthropic para usar esta función. Créala gratis en{" "}
              ...
              <input
                ...
                style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "10px 12px", color: "#e2e8f0", fontFamily: "inherit", fontSize: 13, outline: "none", boxSizing: "border-box", marginBottom: 8 }}
              />
              ...
              {result && (
                <div>
                  ...
                  <div style={{ padding: "12px 14px", background: "rgba(255,255,255,0.04)", borderRadius: 12, marginBottom: 10 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#64748b", marginBottom: 12 }}>{result.porcion}</div>
